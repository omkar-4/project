...
void main(){
    ...
    float wave = fbm(5.5 * uv + uTime/3.0); // change 15.5 to change the noise of edges
    ...
    // gl_FragColor = texture2D(uImage1,uv); // sample the texture
    gl_FragColor = tex; // sample the texture
}